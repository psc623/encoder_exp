...
#SBATCH --output=/net/projects2/litian-lab/scpan/logs/round4_smoke_%j.log
#
# Exercises the round-4 finetune path end to end on the real dataset2 volumes
...
# which is where the round-4 changes are riskiest (deferred selection reading
...
echo "=== round-4 smoke on $(hostname) at $(date -u) job=$SLURM_JOB_ID ==="
nvidia-smi -L

SCRATCH=$(mktemp -d /net/projects2/litian-lab/scpan/encoders/artifacts/.round4_smoke_XXXXXX)
...
result = run_finetune("data/manifests/adni_full_mass_d2_reshuffled_ss.csv",
                      ...
                      warm_start_dir="artifacts/attention/ad/improve3_d2ss70")

selection, history = result["selection"], result["history"]
print("\n=== round-4 finetune smoke ===")
...
if failures:
    ...
    raise SystemExit(1)
print("ROUND-4 SMOKE OK")
PY
STATUS=$?

if [ "$STATUS" -ne 0 ]; then
  echo "=== round-4 smoke FAILED (exit $STATUS); the dependent rerun will not start ===" >&2
  exit "$STATUS"
fi
echo "=== round-4 smoke passed at $(date -u) ==="
